...
=======
      var bufferParams = new BufferParameters({
        ...
        outSpatialReference: view.spatialReference,
        geometries: [tmp]
>>>>>>> f142129d2391088609ef4a271c8702ac4284d67f
      });
...
<<<<<<< HEAD
          ...
          var clickedOnMcd = false;
          var clickedOnCannabisPermit = false;

          var mcdLayerOn;


          /*
            after first click check to see if it is a MCD. if so then just display mcd popup
          */
          ...
          var searchType = 'searchByClick';
          addSearchPolygonToMapHelper(response, searchType, 'test' )

          // getInsideWhatZoning(negativeBufferedGeometry)
          // .then(function (zoningLayer) {
          //   console.log('zoning layer: ', zoningLayer);
          //   SearchCtrl.checkIfParcelIsMcd(mapBlockLotNum)
          //   .then(function (isMcdResponse) {
          //     console.log('mcd response: ', isMcdResponse)
          //     clickedOnMcd = isMcdResponse.features.length !== 0;
          //     SearchCtrl.searchParcelIsCannabisPermit(mapBlockLotNum)
          //     .then(function (isCannabisPermitResponse) {
          //       clickedOnCannabisPermit = isCannabisPermitResponse.features.length !== 0;
          //       if (clickedOnMcd && clickedOnCannabisPermit) {
          //         if (mcdLayerOn) {
          //           // just display top layer?
          //         }
          //         console.log('omg clicked on cannabis permit and mcd')
          //       } else if (clickedOnCannabisPermit) {
          ...
          //         searchPopupHtml = getPopupForSearch(clickedOnMcd, clickedOnCannabisPermit, cannabisPermitAttributes, zoningLayer)
          //         view.popup.open({
          //           content: searchPopupHtml,
          //           location: geometry.extent.center
          //         });
          //       } else if (clickedOnMcd) {
          //         console.log('clicked on mcd')
          //         var mcdAttributes = firstResultFeature.attributes;
          //         searchPopupHtml = getPopupForSearch(clickedOnMcd, clickedOnCannabisPermit, firstResultAttributes, zoningLayer)
          //         view.popup.open({
          //           content: searchPopupHtml,
          //           location: geometry.extent.center
          //         });
...
>>>>>>> f142129d2391088609ef4a271c8702ac4284d67f
    ...
    */
    function addSearchPolygonToMapHelper(jsonData, searchType) {
      ...
      if (searchType === 'searchByClick') {
        firstResult = jsonData.results[0];
        firstResultFeature = firstResult.feature;
        firstResultAttributes = firstResultFeature.attributes;
        geometry = firstResultFeature.geometry;
        tempPolygonHolder = new Polygon(geometry);
        featuresFromJsonResponse = [firstResultFeature];
        correctedFieldsToUse = [];
        ...
        SearchCtrl.checkIfParcelIsMcd(mapBlockLotNum).then(function (isMcdResponse) {
          ...
          SearchCtrl.searchParcelIsCannabisPermit(mapBlockLotNum).then(function (isCannabisPermitResponse) {
            ...
            if (isOnMcd && isOnCannabisPermit) {
              console.log('omg clicked on cannabis permit and mcd')
              /*
                if both layers are on, then just display top layer
              */
              if (mcdLayerOn) {
                // display one popup with multiple selections
                console.log(jsonData);
              ...
              console.log(geometry.extent.center)
              view.popup.open({
                content: searchPopupHtml,
                location: geometry.extent.center
              });
              ...
              console.log('clicked on mcd')
              var mcdAttributes = isMcdResponse.features[0].attributes;
              // console.log(mcdAttributes);
              searchPopupHtml = getPopupForSearch(isOnMcd, isOnCannabisPermit, mcdAttributes, zoningLayer);
              ...
              console.log('just a regular parcel');
              console.log(firstResultAttributes);
